package com.example.ju.streaming.liveVideoBroadcasterSDK;
...
import com.example.ju.streaming.liveVideoBroadcasterSDK.encoder.AudioHandler;
import com.example.ju.streaming.liveVideoBroadcasterSDK.encoder.CameraSurfaceRenderer;
import com.example.ju.streaming.liveVideoBroadcasterSDK.encoder.TextureMovieEncoder;
import com.example.ju.streaming.liveVideoBroadcasterSDK.encoder.VideoEncoderCore;
import com.example.ju.streaming.liveVideoBroadcasterSDK.network.IMediaMuxer;
import com.example.ju.streaming.liveVideoBroadcasterSDK.network.RTMPStreamer;
import com.example.ju.streaming.liveVideoBroadcasterSDK.utils.Resolution;
import com.example.ju.streaming.liveVideoBroadcasterSDK.utils.Utils;
...
public class LiveVideoBroadcaster extends Service implements ILiveVideoBroadcaster, CameraHandler.ICameraViewer, SurfaceTexture.OnFrameAvailableListener {
    ...
    public boolean startBroadcasting(String rtmpUrl) {
        ...
        if (Utils.doesEncoderWorks(context) != Utils.ENCODER_WORKS) {
            ...
            Snackbar.make(mGLView, "Your device is not eligible to broadcast. There is no hardware encoder.", Snackbar.LENGTH_LONG).show();
            return false;
        }
        ...
        new AsyncTask<Integer, Void, Camera.Parameters>() {
            ...
            protected void onPostExecute(Camera.Parameters parameters) {
                if (context.isFinishing()) {
                    ...
                    Snackbar.make(mGLView,"Camera is not running properly", Snackbar.LENGTH_LONG)
                            .show();
                }
    ...
    private int setCameraParameters(Camera.Parameters parameters) {
        ...
        for (int i = 0; i < previewSizeList.size(); i++) {
            ...
            if ((size.width % 960 == 0) && (size.height % 720 == 0)) {
        ...
        Log.e("해상도높이",Integer.toString(previewSize.getHeight()));
        Log.e("해상도넓이",Integer.toString(previewSize.getWidth()));



//        Toast.makeText(context, Integer.toString(size.width)+",,,"+Integer.toString(size.height), Toast.LENGTH_SHORT).show();
    ...
    public void showEncoderNotExistDialog() {
        mAlertDialog = new AlertDialog.Builder(context)
                //.setTitle("")
                .setMessage("Your device is not eligible to broadcast. There is no hardware encoder")
                .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                    }
                })
    ...
    public void changeCamera() {
        if (!getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_FRONT)) {
            Snackbar.make(mGLView,"Only one camera exists in this device", Snackbar.LENGTH_LONG).show();
            return;
        }
        if (sCameraProxy == null) {
